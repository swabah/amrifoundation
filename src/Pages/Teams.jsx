  ...
  return (
    <div className='w-full h-full'>
      <div className='flex flex-col items-center justify-center w-full h-full bg-white space-y-7 lg:space-y-12 py-7 md:py-16 lg:py-22'>
        ...
        <p className='px-5 font-sans md:px-10 lg:px-10 xl:px-36 mt-4 text-center text-[#555555] text-base md:text-xl lg:text-2xl font-normal leading-snug tracking-wide'>Our team comprises diverse and passionate individuals, each bringing unique skills, experiences, and perspectives to collectively drive our mission forward with unwavering dedication</p>
      </div>

      <div style={{ backgroundImage: `url(${PatternBg})` }} className='relative flex flex-col items-center justify-center w-full h-full gap-10 px-5 py-16 bg-no-repeat md:py-20 lg:py-24 lg:gap-16 xl:gap-20 md:px-10 lg:px-10 xl:px-36'>
        ...
        <div className='z-10 grid w-full h-full grid-cols-2 gap-3 md:grid-cols-4 lg:gap-5 grid-rows-auto'>
          {Leadershipdata.map((item) => (
            <div className='w-full h-auto' onClick={() => showEventModal(item.id , item.name)}>
      ...
      <div className='flex flex-col w-full h-full gap-10 px-5 py-16 text-center lg:gap-20 md:px-10 lg:px-10 xl:px-36 md:py-20 lg:py-24'>
